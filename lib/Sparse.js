    ...
    while (currentArgIndex < argvCount) {
      ...
      if (!name) {
        ...
        if (optGroupArray.length == 1) {
          const shorthand = optGroupArray[0];
          if (this.#isValidOption(shorthand)) {
            name = this.#getOptionNameFromShorthand(shorthand);
          } else {
            const message = `'${shorthand}' is not a valid option.`;
            this.#addError(message);
          }
        } else {
